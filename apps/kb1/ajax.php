...
else if ( $method == 'UPLOAD' )
{
	...
	if ($_FILES["fileupload"]["size"] > UPLOAD_MAXSIZE)
		...
		$r["error_text"] = "Error code: " . $_FILES["fileupload"]["error"] ;
	else {
		do {
			$num = dechex(rand(1,100000000));
			$fn = "file" . sprintf ('%08s', $num) . "." . pathinfo($_FILES["fileupload"]["name"], PATHINFO_EXTENSION);
		...
		if(!move_uploaded_file($_FILES["fileupload"]["tmp_name"],$uploadFn)) {
			switch ($_FILES['uploadedfile'] ['error']) {
				case 1:
					$r["error_text"] = 'The file is bigger than this PHP installation allows';
					break;

				case 2:
					$r["error_text"] = 'The file is bigger than this form allows';
					break;

				case 3:
					$r["error_text"] = 'Only part of the file was uploaded';
					break;

				case 4:
					$r["error_text"] = 'No file was uploaded';
					break;

				default:
					$r["error_text"] = "{$_FILES['fileupload'] ['error']}";
					break;
			}
			$r["error_text"] = "There was an error uploading the file, please try again! -- File - {$_FILES['fileupload']['tmp_name']}    Destination - {$uploadFn}     Error - {$r['error_text']}" ;
		}else{	
			$pageURL = 'http';
			if ($_SERVER["HTTPS"] == "on")
				$pageURL .= "s";
