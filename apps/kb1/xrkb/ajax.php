...
function PostToHost($url, $data) {
	if (substr($url, 0, 1) != ".") {
		// patch up for missing protocol (assume http)
		if (strrpos($url, "://") === false) {
			$url = "http://" . $url;
		}
	} else {
		// local file, do some jiggery pokery
		$p = (isset($_SERVER['HTTPS']) && ($_SERVER['HTTPS'] != "off") ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
		// lose the ajax.php
		$p = substr($p, 0, strlen($p) - 8);
		// append the passed url and a level up (as ajax.php lives in the xrkb sub folder)
		$p .= "../" . $url;
		//
		$url = $p;
	}
...
else if ( $method == 'UPLOAD' )
{
	$r = array("error" => true,
		"error_text" => "",
		"location" => "");
	if ($_FILES["afile"]["size"] > UPLOAD_MAXSIZE)
		$r["error_text"] = "File too large!" ;
	else if ($_FILES["afile"]["error"] > 0) 
		$r["error_text"] = "Error code: " . $_FILES["afile"]["error"] ;
	else {
		do {
			$num = dechex(rand(1,100000000));
			$fn = "file" . sprintf ('%08s', $num) . "." . pathinfo($_FILES["afile"]["name"], PATHINFO_EXTENSION);
			$uploadFn = UPLOAD_PATH . "/" . $fn ;
		} while ( file_exists($uploadFn) );
		if ( !file_exists(UPLOAD_PATH) )
			mkdir(UPLOAD_PATH);
		move_uploaded_file($_FILES["afile"]["tmp_name"],$uploadFn);
